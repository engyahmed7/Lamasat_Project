...
const SingleProject = () => {
  ...
  const getProject = async () => {
    const { data } = await axios.get(
      `https://api.lamasat.cloud/api/v1/projects/show/${id}`
    );
    ...
    <>
      ...
      <section className="mt-10">
        ...
        <p className="px-5 text-gray-600 text-sm mb-11 break-words ">
          {i18n.language === "en" ? project?.description?.en : project?.description?.ar}
        </p>
        <LightGallery
        ...
        >
          {project?.images?.map((image) => (
            <a
              key={image.id}
              data-pinterest-text={`https://api.lamasat.cloud${image.photo}`}
              data-facebook-share-url={`https://api.lamasat.cloud${image.photo}`}
              data-tweet-text={`https://api.lamasat.cloud${image.photo}`}
              data-src={`https://api.lamasat.cloud${image.photo}`}
            >
              <img
